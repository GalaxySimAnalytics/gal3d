...
class Gal3DAnalyzer:
    ...
    def __init__(self, particle: Particles, field: SphField, structure: Structure3D, optimizer: Optimizer):
...
def get_ell_structure(self: Gal3DAnalyzer,a: float,**kwargs) -> ModelResult:
    ...
    if (self.structure._geometry_name == 'Ellipsoid') or (self.structure._geometry_name == 'Ellipsoid_S' and fitonce):
    ...
    if self.structure._geometry_name == 'Ellipsoid_S':
        ...
        ellipsoid = Structure3D(self.structure._coordinate,"Ellipsoid",
        ...
        if init_parameters:
            updatevalue = {i:init_parameters[i] for i in init_parameters.keys() & params_ell.keys()}
        ...
        ell_res = self.optimizer.fitting(fun,list(x0_dict.values()),bounds,func_kwargs=dict(**data))
    ...
    op_res = self.optimizer.fitting(fun,list(x0_dict.values()),bounds,func_kwargs={**data})
