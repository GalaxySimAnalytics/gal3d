...
def _get_config_parser_with_defaults():
    ...
    import configparser
    config_parser = configparser.RawConfigParser(comment_prefixes = ('#',';','---'),inline_comment_prefixes=('#'),
...
def _get_basic_config_from_parser(config_parser):
    config = {'update_stub': config_parser.getboolean('general','update_stub',fallback=False)}
...
logo=f"""
  ...
  |   :    /   ;  :   .'   \ |  ,   /  \ ''\        ;  ;   :  .'      
   \   \ .'    |  ,     .-./  ---`-'    \   \     .'   |   ,.'        
    `---`       `--`---'                 `--`-,,-'     '---'          """
logo_color = "\n".join([
    string_formator(logo.split('\n')[i],bg_color=(0+5*i,0+5*i,0+5*i),fg_color='white',bold=True,italics=True) 
