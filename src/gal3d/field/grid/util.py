...
@njit(types.Tuple((float64[:,:],float64[:,:]))(float64[:],float64[:]),cache=True,)
def split_byequalvolumn(posmin,posmax):
    #half_boxsize = (posmax - posmin)/2
    cent_pos = (posmin+posmax)/2.
...
@njit(types.Tuple((float64[:,:],float64[:,:]))(float64[:],float64[:],float64[:,:]),parallel=True,fastmath=True,cache=True,)
...
@njit(int64(float64[:],float64[:],float64[:,:],int64[:],int64,int64),parallel=True,nogil=True,fastmath=True,cache=True,)
...
@njit(int64(float64[:],float64[:],float64[:,:],int64[:],int64),parallel=True,nogil=True,fastmath=True,cache=True,)
...
@njit(types.Tuple((float64[:],float64[:],float64[:]))(float64[:,:],float64[:,:],float64[:],int64[:]),parallel=True,nogil=True,fastmath=True,cache=True,)
...
@njit(int64[:](float64[:,:],float64[:,:],float64[:,:],int64[:],int64,int64[:]),parallel=True,nogil=True,fastmath=True,)
...
@njit(types.Tuple((float64[:,:],float64[:,:],int32[:],int64[:],int64[:]))(float64[:,:],int32,int32,float64[:,:],float64[:,:]))
def make_grid_by_num(pos,maxdepth,cut_max_partnum,lower_pos,upper_pos):
    ...
    while True:
        ...
        while (Nums[i] > cut_max_partnum) and (Depth[i] < maxdepth):
            ...
            add_lower, add_upper = split_byequalvolumn(lower_pos[i],upper_pos[i])   # 7 new
            ...
            nums_add = set_partical_nums(posmin = add_lower, posmax = add_upper,
                                    pos = pos, indices= Indice, parent_ind=i, child_ind=child_inds)
...
@njit(types.Tuple((float64[:,:],float64[:,:],int32[:],int64[:],int64[:]))(float64[:,:],int32,int32,float64[:,:],float64[:,:]),cache=True,)
def make_grid_by_diff(pos,maxdepth,cut_diff_partnum,lower_pos,upper_pos):
    ...
    while i <= (len(Depth)-1) :
        ...
        while (Depth[i] < maxdepth) and flag:
            ...
            add_lower, add_upper = split_by_median_point(lower_pos[i],upper_pos[i],pos)   # 7 new #TODO median ?
            
            ThisNums = np.zeros(8,dtype=np.int64)
            for grid_ind in range(len(add_lower)):
                ThisNums[grid_ind] = test_partical_num(posmin = add_lower[grid_ind], posmax = add_upper[grid_ind],
                                    pos = pos,indices= Indice, parent_ind=i)
            ...
            if (np.max(ThisNums) < cut_diff_partnum) or np.min(Vs)<1e-3:
               # print(((np.sum(sortNum[4:])-np.sum(sortNum[:4]))/4),cut_diff_partnum)
                ...
                for grid_ind in range(len(add_lower)):
                    if grid_ind == 0 :
                        continue
                    ThisNums[grid_ind] = set_partical_num(posmin = add_lower[grid_ind], posmax = add_upper[grid_ind],
                                        pos = pos,indices= Indice, parent_ind=i,child_ind=len(Depth)+grid_ind - 1)
