...
class SphField:
    ...
    def __init__(self, particles: Particles, num_ray=1024,ray_method = 'fibonacci', **kwargs):
        '''
    ...
    @timing
    def build_profile_sample(self, num_p: int = 500, step_mode: str = 'log',):
        '''
        ...
        self.points_parameter = self.particles.get_parameter(points_que).reshape(self.points_r.shape)
        return self
    
    
    @timing
    def build_profile_interpolator(self, interpolator_method = 'LU',f_de=True,interpolator_kwargs=dict(),**kwargs):
        '''
        ...
        **kwargs : dict
        ...
        '''
        self.rays_func = [MonotonRay(self.points_r[i],self.points_parameter[i],f_de=f_de,
    ...
    @timing
    def build_isodensity_profile(self, method: str = 'pair', from_rays_func: bool = False, res_b: float = 0.2, res_c: float = 0.1,**kwargs):
        '''
    ...
    def set_isodensity_sphere(self,from_rays_func = False,**kwargs):
        ...
        **kwargs : dict
        ...
        if from_rays_func:
            ...
            self.iso_parameters = self.particles.get_parameter(self.iso_points.reshape(self.iso_points.shape[0]*self.iso_points.shape[1],3)).reshape(
    ...
    def gradient_ray_n(self,n:int)->tuple:
        ...
        '''
        return ((self.particles.gradient[0][0][self.points_index[n]],self.particles.gradient[0][1][self.points_index[n]]),
                (self.particles.gradient[1][0][self.points_index[n]],self.particles.gradient[1][1][self.points_index[n]]))
    ...
    def generate(self, r: Iterable | float, for_fit = False,**kwargs)->dict:
        ...
        if for_fit:
            ...
            Eq_surface['r'] = Eq_surface['r']/np.sqrt(np.sum(Eq_surface['r']**2)/len(Eq_surface['r']))   #  normalization as this used for calculate error
...
def bound_value(cls,value,mode='max',**kwargs):
    ...
    **kwargs : dict
    ...
    if mode =='max':
        return np.array([np.max(cls.r_ray_n(i)[cls.parameter_ray_n(i)>value]) for i in range(cls.rays.num)])
    if mode =='min':
        return np.array([np.min(cls.r_ray_n(i)[cls.parameter_ray_n(i)<value]) for i in range(cls.rays.num)])
