...
def sg_smooth(
    x, smooth_log, window_length, polyorder: int = 1, mode: str = 'nearest', **kwargs
):
    '''
    ...
    **kwargs : dict
    ...
    if smooth_log:
        ...
        new_x = savgol_filter(
            x, window_length=window_length, polyorder=polyorder, mode=mode, **kwargs
        )
    return new_x


def sg_smooth_tomono(
    x, smooth_log, polyorder: int = 1, mode: str = 'nearest', mono_de: bool = True
):
    '''
...
def sg_smooth_throw(
    ...
    while flag is False:
        ...
        smoothed = sg_smooth(
            x_smooth,
            smooth_log=smooth_log,
            window_length=window_length_max,
            polyorder=polyorder,
            mode=mode,
        )
