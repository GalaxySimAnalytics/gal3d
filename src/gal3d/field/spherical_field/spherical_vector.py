...
class SphSampler:
    ...
    @staticmethod
    def muller_sampling(Num_sampling: int = 256) -> tuple[np.ndarray,np.ndarray]:
        """
        ...
        u = rng.normal(size=Num_sampling)
        v = rng.normal(size=Num_sampling)
        w = rng.normal(size=Num_sampling)
    ...
    def thomson_sampling(Num_sampling: int = 256, iterations: int = 300) -> tuple[np.ndarray,np.ndarray]:
        ...
        rng = np.random.default_rng(42)  # For reproducibility
        pos = rng.normal(size=(Num_sampling, 3))
        ...
        for i in range(iterations):
            ...
            if i % 10 == 0 or i == iterations - 1:  # Check periodically to save computation
                ...
                logger.debug("Thomson iteration %d, uniformity: %.4f, best: %.4f", i, uniformity, best_uniformity)
