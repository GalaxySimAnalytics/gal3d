...
class OptimizerScipy(OptimizerBase):
    ...
    def fitting(self,fun,x0,bounds,func_args: tuple | None = None,func_kwargs: dict | None = None,**kwargs):
        ...
        res = optimize.minimize(fun=fn,x0=x0, method=self.algo_name,bounds=bounds,options=self.algo_options,**kwargs)
        return process_scipy_result(res)

    @classproperty
    def available_algorithm(cls):
        return ['Nelder-Mead','Powell','CG','BFGS','Newton-CG','L-BFGS-B','TNC','COBYLA','COBYQA'
