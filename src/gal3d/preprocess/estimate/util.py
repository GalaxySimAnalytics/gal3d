...
@njit(types.Tuple((float64[:],float64,float64,int32))(float64[:,:],float64[:],int32,int32,float64,float64,int32),nogil=True,parallel=True,fastmath=True,cache = True)
def shrink_sphere_center(pos,weight,min_points,particles_for_second_radius,shrink_factor,starting_rmax,itermax):
    ...
    while True :
        ...
        if npart < particles_for_second_radius and second_radius < 0.:
...
@njit(float64[:](float64[:,:],float64[:]), nogil=True,parallel=True,fastmath=True, cache =True)
...
def moment_of_inertia(pos, m):
    return np.array([[np.sum(m * pos[:, i] * pos[:, j]) for j in prange(3)] for i in prange(3)])/np.sum(m)
...
def abc_vect(pos,pa):
    ...
    return abc,np.dot(D[1].T[align],axis)       # make np.eye(3), to abc, Rotate(np.eye(3),avc_vect.T)
