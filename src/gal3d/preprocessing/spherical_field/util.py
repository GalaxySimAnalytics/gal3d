...
from ...util.array_operate import vector_length3d,unit_vector3d,trans_to_Spherical_coordinates,trans_to_Cartesian_coordinates,Matmul,Dot,Rotate
...
def fibonacci_sampling( Num_sampling: int =256):
    ...
    for n in prange(Num_sampling):
        i = n + 1
        sampling_pos[n][0] = math.sqrt(1 - ((2*i - 1)/Num_sampling-1)**2)*math.cos(2*math.pi*i*Golden_Ratio)
        sampling_pos[n][1] = math.sqrt(1 - ((2*i - 1)/Num_sampling-1)**2)*math.sin(2*math.pi*i*Golden_Ratio)
