...
class Structure3D:
    ...
    def __init__(self, coordinate: CoordinateBase | str, geometry: GeometryBase | str, error_func: Callable | str,
        ...
        self.parameters = self._coordinate.get_parameters() + self._geometry.get_parameters()
    ...
    def init_parameters(self,*args,**kwargs) ->Parameters:
        if args:
            ...
            return self._coordinate.init_parameters(**params) + self._geometry.init_parameters(**params)
        if kwargs:
            return self._coordinate.init_parameters(**kwargs)+self._geometry.init_parameters(**kwargs)
    ...
    def _set_geometry(self, geometry: GeometryBase | str):
        ...
        assert (isinstance(geometry,GeometryBase) or isinstance(geometry,str) or issubclass(geometry,GeometryBase))
        self._geometry = Geometry.get_plugin(geometry) if isinstance(geometry,str) else geometry
        self._geometry_name = self._geometry.__name__

    
    def _set_coordinate(self, coordinate: CoordinateBase | str):
        
        assert (isinstance(coordinate,CoordinateBase) or isinstance(coordinate,str) or issubclass(coordinate,CoordinateBase))
        self._coordinate = Coordinate.get_plugin(coordinate) if isinstance(coordinate,str) else coordinate
    ...
    def _set_error_func(self, error_func: Callable | str):
        ...
        self._error_params = list(filter(lambda x: ('_call' not in x) and ('_call' not in x), self._error_params)) 
    ...
    def _set_error_method(self, error_method: Callable | str):
        ...
        self._error_method = MethodType(self._compute_error_method[error_method],self) if isinstance(error_method,str) else error_method
    ...
    def __eq__(self, other):
        ...
        if (isinstance(other,Structure3D) 
    ...
    def ray_intersect(self, pos, **kwargs) -> tuple:
        ...
        return self._geometry(**geoty_pa).ray_intersect(self._coordinate(**coord_pa)(pos))
    ...
    def line_intersect(self, pos1: ArrayLike, pos2: ArrayLike, **kwargs) -> np.ndarray:
        ...
        return self._geometry(**geoty_pa).line_intersect(pos1 =self._coordinate(**coord_pa)(pos1), pos2=self._coordinate(**coord_pa)(pos2))
    ...
    def quick_f_ray_d(self,  *args,pos,**kwargs) -> np.ndarray:
        ...
        return self._geometry.quick_f_ray_d(**geoty_pa, pos = self._coordinate.quick_call(**coord_pa,pos = pos))
    ...
    def _generate_normal(self,**kwargs) -> Tuple[dict,dict]:
        coord_pa = self._coordinate.init_parameters(**kwargs) if kwargs else self.parameters
        geoty_pa = self._geometry.init_parameters(**kwargs) if kwargs else self.parameters
...
def isodensity_fcall(self: Structure3D , params: tuple | ArrayLike, **kwargs) -> float:
    ...
    f_call = self.quick_call(*params,pos=kwargs['pos'])-1.
    error_pa = {i: kwargs[i] for i in self._error_params}
    
    return self._error_func(f_call,**error_pa)

@Structure3D.compute_method_registry
def isodensity_dcall(self: Structure3D , params: tuple | ArrayLike,*args, **kwargs) -> float:
        
    f_call = self.quick_f_ray_d(*params,pos=kwargs['pos'])-1.
    error_pa = {i: kwargs[i] for i in self._error_params}
