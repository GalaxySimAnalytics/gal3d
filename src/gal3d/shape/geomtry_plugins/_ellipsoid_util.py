...
__all__ = ["f_ellipsoid","f_ellipsoid_jacobian","f_ellipsoids",
           ...
           "DistancePointsEllipsoid","IntersectRaysEllipsoid","IntersectLinesEllipsoid","f_ray_ellipsoid"]
...
@jit(types.Tuple((float64[:],float64[:],float64[:],float64[:],float64[:],float64[:]))
     (float64,float64,float64,float64[:,:]),nogil=True,parallel=True,fastmath=True,cache=True)
...
@jit(types.Tuple((float64[:],float64[:],float64[:]))(float64,float64,float64[:],float64[:],int64),nogil=True, parallel=True,cache=True )  
...
@jit(types.Tuple((float64[:],float64[:],float64[:],float64[:]))
     (float64, float64, float64, float64[:], float64[:], float64[:], int64),nogil=True,parallel=True ,cache=True )
...
def IntersectRaysEllipsoid(a, b, c, pos):
    ...
    for i in prange(len(pos)):
        tarpos[i,0],tarpos[i,1],tarpos[i,2],d[i], L[i] = IntersectRayEllipsoid(a,b,c,pos[i,0],pos[i,1],pos[i,2])
    return tarpos,L-d
...
@jit(float64[:,:](float64,float64,float64,float64[:,:],float64[:,:]),nogil=True,parallel=True,fastmath=True,cache=True)
