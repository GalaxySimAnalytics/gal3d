...
class Ellipsoid_S(GeometryBase):
    ...
    PN = ('a','eps_ab','eps_bc','sa','sb','sc')   ## not use set !!!##
    ...
    def ray_intersect(self, pos: ArrayLike) -> tuple:
        ...
        if len(np.shape(pos))==1:
            pos = np.float64([pos])
        return IntersectRaysEllipsoid_S(self['a'],self['b'],self['c'],self['sa'],self['sb'],self['sc'], pos, Ellipsoid_S.MaxIterationDist)
    ...
    def f_ray_d(self, pos: ArrayLike) -> np.ndarray:
        ...
        if len(np.shape(pos))==1:
            pos = np.float64([pos])
        return f_ray_shaped_ellipsoid(self['a'],self['b'],self['c'],self['sa'],self['sb'],self['sc'], pos, Ellipsoid_S.MaxIterationDist)
    ...
    def quick_f_ray_d(a,eps_ab,eps_bc,sa,sb,sc,pos) -> np.ndarray:
        ...
        b = a*(1-eps_ab)
        c = b*(1-eps_bc)
        return f_ray_shaped_ellipsoid(a,b,c,sa,sb,sc,pos,Ellipsoid_S.MaxIterationDist)
    ...
    def quick_ray_dist(a,eps_ab,eps_bc,sa,sb,sc,pos) -> np.ndarray:
        ...
        b = a*(1-eps_ab)
        c = b*(1-eps_bc)
        return IntersectRaysEllipsoid_S(float(a),b,c,sa,sb,sc,pos,Ellipsoid_S.MaxIterationDist)[1]
