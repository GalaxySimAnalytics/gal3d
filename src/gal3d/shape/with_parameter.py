    ...
    def __init_subclass__(cls, **kwargs) -> bool:
        for key,typ in zip(required_attrs,required_type):
            if not hasattr(cls,key):
                logger.warning(f"{cls.__name__} does not define the {key} with type of {typ}")
                return False
            if not isinstance(getattr(cls,key),typ):
                logger.warning(f"{cls.__name__} does define the {key} with type of {type(getattr(cls,key))}, need {typ}")
                return False
        return True
