...
class Auto3DShape:
    ...
    def to_3d_array(pos: ArrayLike) -> NDArray:
        ...
        arr = np.array(pos)
        if arr.ndim != 2:
            logger.debug("pos is 1d array with shape= %s, reshaping to (-1,3)", repr(arr.shape))
            arr = arr.reshape(-1, 3)
        if arr.shape[1] == 3:
            return arr
        if arr.shape[0] == 3:
            logger.debug("pos have the shape= %s, transposing it", repr(arr.shape))
            return arr.T
        logger.debug("pos have the shape= %s, target shape: (n,3), reshaping it", repr(arr.shape))
