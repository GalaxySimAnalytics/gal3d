...
@jit(float64[:,:](float64[:,:],float64[:,:]),nogil=True, fastmath=True,cache=True,)
def Rotate(pos,mat):
...
def trans_to_Cartesian_coordinates(sphere_coor):
    ...
    pos_data[:,0] = sphere_coor[:, 0]*np.sin(sphere_coor[:, 1])*np.cos(sphere_coor[:, 2])
    pos_data[:,1] = sphere_coor[:,0]*np.sin(sphere_coor[:, 1])*np.sin(sphere_coor[:, 2])
