...
class ModelProjectorBase(abc.ABC):
    ...
    @ImageCache
    def image(self,x_range,y_range, nbins: int = 100, z_range: Sequence = (-20,20), rotation: None | NDArray[np.float64] = None,**kwargs):
    ...
    @abc.abstractmethod
    def _image(self, x_range,y_range, nbins: int = 100, z_range: Sequence =(-20,20), rotation: None | NDArray[np.float64] = None,**kwargs):
        pass

    def image_xz(self, x_range,y_range,nbins: int = 100, z_range: Sequence =(-20,20), ):
        return self.image(x_range,y_range,nbins, z_range, rotation = np.array([[1.,0,0],[0,0,1.],[0,1.,0.]]).T)
    
    def image_yz(self, x_range,y_range,nbins: int = 100, z_range: Sequence =(-20,20), ):
